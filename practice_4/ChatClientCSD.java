...
/*/
      ...
      */
       boolean status = srv.connectUser(myUser);
       if (status == false) {
   ...
   public void doLeaveChannel (String channelName) throws Exception {
      ...
      //3.b Make that the user "myUser" leaves the channel "ch".
        boolean status = ch.leave(myUser);
        if (status == false) {
              throw new Exception("User is not in channel");
        }
      }
   }
